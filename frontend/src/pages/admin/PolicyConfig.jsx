    ...
    <div className={`card transition-all ${!local.enabled ? 'opacity-60' : ''}`}>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-1.5">
          <label className="text-sm text-bark font-medium">Confidence Threshold</label>
      ...
      <div className="mb-4">
        <label className="text-sm text-bark font-medium block mb-2">Enforcement</label>
            ...
            <button
              ...
              className={`px-3 py-2 rounded text-sm font-medium border transition-colors
                ${local.enforcementBehavior === b
                  ...
                  : 'border-fog text-silver hover:border-bark hover:text-bark'}
      ...
      <div className="flex items-center justify-between pt-3 border-t border-fog">
        <p className="text-xs text-silver">
          {policy.updatedBy
            ? `Last changed by ${policy.updatedBy.email}`
            : 'Default (system seed)'}
...
export default function PolicyConfig() {
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-bark">Policy Configuration</h1>
      ...
      <div className="space-y-4">
        {policies.map((p) => (
          <PolicyCard key={p.category} policy={p} onSaved={handleSaved} />
        ))}
      </div>
    </div>
  );
